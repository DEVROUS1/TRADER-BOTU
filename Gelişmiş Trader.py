...
if __name__ == "__main__":
    try:
        logger.info("=" * 60)
        logger.info("🚀 ULTIMATE CRYPTO BOT - UPGRADED VERSION")
        logger.info("=" * 60)
        logger.info("✨ YENİ ÖZELLİKLER:")
        logger.info("   • 📊 İnteraktif Plotly Grafikler")
        logger.info("   • 🛡️ Otomatik Stop Loss / Take Profit")
        logger.info("   • 💾 SQLite Veri Kalıcılığı")
        logger.info("   • ⚡ Cache Sistemi (Daha Hızlı)")
        logger.info("   • 🎨 Telegram Inline Keyboard UI")
        logger.info("   • 📈 Risk Yönetimi (Max Drawdown, Position Sizing)")
        logger.info("   • 🔧 Dinamik Risk Ayarları")
        logger.info("=" * 60)

        # Database init
        Database.init_db()
        
        # Geçmiş sinyalleri yükle (Startup'ta)
        SIGNAL_HISTORY = Database.get_recent_signals(limit=50)
        logger.info(f"📂 {len(SIGNAL_HISTORY)} geçmiş sinyal yüklendi.")
        
        # GitHub Actions / RUN_ONCE check
        if IS_CI_MODE:
            logger.info("🤖 CI/One-shot mode detected. Running synchronously without Telegram polling.")
            main_loop()
            # After main_loop returns, the process will exit naturally
        else:
            # Normal mode: Run main_loop in background and start Telegram polling
            # Arka plan thread
            ...
            try:
                ...
                if e.error_code == 409:
                    logger.error("❌ TELEGRAM ÇATIŞMA HATASI (409): Bu bot şu an başka bir yerde (başka bir terminalde veya GitHub Actions üzerinde) zaten çalışıyor olabilir.")
                    logger.error("💡 Lütfen diğer bot pencerelerini kapatın veya GitHub Actions iş akışının bitmesini bekleyin.")
                else:
